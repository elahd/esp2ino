...
function appendFlashStatus (jsonObj) {
  ...
  const currentStepUpdateNode = document.getElementById('step-' + jsonObj.step + '-update')
...
function buildFlashTaskTable () {
  ...
  flashTasks.forEach((task, index) => {
    ...
    switch (task.type) {
      case 'intermediate':
        taskIcon.innerHTML = spinnerNodeInnerHtml + successNodeInnerHtml + warnNodeInnerHtml + failNodeInnerHtml
        break
      case 'done':
